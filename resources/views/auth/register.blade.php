...
<body>
    <div class="auth-container">
        <div class="auth-header">
            <h1><i class="fas fa-user-plus"></i> ثبت‌نام در فروشگاه</h1>
        ...
        <div class="auth-body">
            <div class="step-indicator">
                <div class="step active" id="step1-indicator">
                    ...
                    <div class="step-label">شماره موبایل</div>
                </div>
                <div class="step" id="step2-indicator">
                    <div class="step-circle">۲</div>
                    <div class="step-label">تایید کد</div>
                </div>
            ...
            <div class="form-step active" id="step1">
                <div class="alert alert-info">
                    ...
                    <span>لطفاً شماره موبایل خود را وارد کنید</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="password-requirements">
                        شماره موبایل باید ۱۱ رقمی و با ۰۹ شروع شود
                    </div>
                </div>
                ...
                <div class="messages-container" id="messages-container-1">
                    <div class="alert alert-error" id="error-message-1" style="display: none;"></div>
                    <div class="alert alert-success" id="success-message-1" style="display: none;"></div>
                    <div class="alert alert-user-error" id="user-error-message-1" style="display: none;"></div>
                ...
                <div class="auth-links" style="margin-top: 20px; text-align: center;">
                    <a href="{{ route('login') }}" class="auth-link">
                        ...
                        <span>قبلاً حساب دارید؟ وارد شوید</span>
                    </a>
                </div>
            </div>
            ...
            <div class="form-step" id="step3">
                <div class="alert alert-success">
                    <i class="fas fa-check-circle"></i>
                    <span>کد تایید صحیح است. لطفاً اطلاعات خود را تکمیل کنید</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        ...
                        <input type="text" id="first-name-input" class="form-input" 
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        <i class="fas fa-user input-icon"></i>
                        <input type="text" id="last-name-input" class="form-input" 
        ...
        function restoreState() {
            ...
            if (savedState) {
                try {
                    ...
                    isFromRegistration = state.isFromRegistration || false;
        ...
        function startTimer(forceRestart = false) {
            ...
            if (resendLinkElement) {
                resendLinkElement.classList.add('disabled');
            }
            ...
            timerInterval = setInterval(() => {
                ...
                if (timeLeft <= 0) {
                    ...
                    if (resendLinkElement) resendLinkElement.classList.remove('disabled');
                    
                    otpRequestTime = null;
                    
                    // وقتی تایمر صفر شد، مخفی کردن دکمه بازگشت به تایید کد
                    const goToOtpBtn = document.getElementById('go-to-otp-from-step1');
                    if (goToOtpBtn) goToOtpBtn.style.display = 'none';
                    
                    // فعال کردن دکمه ارسال
                    const sendOtpBtn = document.getElementById('send-otp-btn');
                    if (sendOtpBtn) sendOtpBtn.disabled = false;
                    
                    // اگر در صفحه تایید کد هستیم، به صفحه اول برو
                    if (currentStep === 2) {
                        setTimeout(() => {
                            goToStep(1);
                            showWarning('زمان تأیید کد به پایان رسید. لطفاً مجدداً اقدام کنید.', 2);
                        }, 1000);
                    }
        ...
        function showError(message, step = currentStep) {
            const errorDiv = document.getElementById(`error-message-${step}`);
            const successDiv = document.getElementById(`success-message-${step}`);
            const warningDiv = document.getElementById(`warning-message-${step}`);
            const userErrorDiv = document.getElementById(`user-error-message-${step}`);
        ...
        function showUserError(message, step = currentStep) {
            const userErrorDiv = document.getElementById(`user-error-message-${step}`);
            const errorDiv = document.getElementById(`error-message-${step}`);
            const successDiv = document.getElementById(`success-message-${step}`);
            const warningDiv = document.getElementById(`warning-message-${step}`);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            sendOtpBtn.addEventListener('click', function() {
                ...
                .then(response => {
                    console.log('Register OTP Response Status:', response.status);
                    
                    if (response.status === 422) {
                        // شماره تکراری - قبلاً ثبت‌نام کرده
                ...
                .then(data => {
                    console.log('Register OTP Data:', data);
                    ...
                    if (data.status === 'success') {
                        ...
                        const errorMessage = data.message || 'خطا در ارسال کد تایید';
                ...
                .catch(error => {
                    ...
                    if (error.message === 'MOBILE_DUPLICATE') {
                        showUserError('شما قبلاً با این شماره موبایل ثبت‌نام کرده‌اید. لطفاً وارد حساب کاربری خود شوید.', 1);
                    } else if (error.message.includes('network') || error.message.includes('fetch')) {
                        showError('خطا در ارتباط با سرور. لطفاً اتصال اینترنت خود را بررسی کنید.', 1);
                    } else if (error.message.includes('500')) {
                        showError('خطای داخلی سرور. لطفاً مجدداً تلاش کنید.', 1);
                    } else {
                        showError('خطای نامشخص در سیستم. لطفاً با پشتیبانی تماس بگیرید.', 1);
                    }
                })
                .finally(() => {
            ...
            verifyOtpBtn.addEventListener('click', function() {
                ...
                if (!/^\d{7}$/.test(otp)) {
                    showError('کد تایید اشتباه است. لطفاً کد صحیح ۷ رقمی را وارد کنید', 2);
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        // خطای OTP اشتباه
                        const errorMessage = data.message || 'کد تایید اشتباه است. لطفاً کد صحیح را وارد کنید';
                ...
                .catch(error => {
                    ...
                    showError('کد تایید اشتباه است. لطفاً کد صحیح را وارد کنید', 2);
                    otpInput.value = '';
                    otpInput.focus();
                })
                .finally(() => {
            ...
            registerBtn.addEventListener('click', function() {
                ...
                if (firstName.length < 2) {
                    showError('لطفاً نام خود را وارد کنید (حداقل ۲ کاراکتر)', 3);
                    firstNameInput.classList.add('error');
                    return;
                }
                
                if (lastName.length < 2) {
                    showError('لطفاً نام خانوادگی خود را وارد کنید (حداقل ۲ کاراکتر)', 3);
                ...
                registerBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i><span>در حال ثبت‌نام...</span>';
                ...
                fetch("{{ route('register.complete') }}", {
                    ...
                    body: JSON.stringify({
                        mobile: mobileNumber,
                        name: fullName, // ارسال نام کامل
                        first_name: firstName, // ارسال نام جداگانه (برای مدل جدید)
                        last_name: lastName, // ارسال نام خانوادگی جداگانه
                        password: password,
                ...
                .then(data => {
                    if (data.status === 'success') {
                        showSuccess('ثبت‌نام با موفقیت انجام شد. در حال ورود به سیستم...', 3);
                        ...
                        showError(data.message || 'خطا در ثبت‌نام', 3);
                        registerBtn.disabled = false;
                        registerBtn.innerHTML = '<i class="fas fa-user-check"></i><span>تکمیل ثبت‌نام</span>';
                    }
                })
                .catch(error => {
                    console.error('Register Complete Error:', error);
                    showError('خطا در ارتباط با سرور', 3);
                    registerBtn.disabled = false;
                    registerBtn.innerHTML = '<i class="fas fa-user-check"></i><span>تکمیل ثبت‌نام</span>';
                });
            });
            ...
            setTimeout(() => {
                ...
                // اگر از ثبت‌نام آمده‌ایم و شماره قبلی داریم
                if (isFromRegistration && lastSentMobile && otpSent && remainingCooldown > 0) {
                    // بررسی URL قبلی
